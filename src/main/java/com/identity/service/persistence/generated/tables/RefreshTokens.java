...
public class RefreshTokens extends TableImpl<RefreshTokensRecord> {
    ...
    /**
     * The column <code>public.refresh_tokens.token_family_id</code>.
     */
    public final TableField<RefreshTokensRecord, UUID> TOKEN_FAMILY_ID = createField(DSL.name("token_family_id"), SQLDataType.UUID.nullable(false), this, "");

    /**
     * The column <code>public.refresh_tokens.status</code>.
     */
    public final TableField<RefreshTokensRecord, String> STATUS = createField(DSL.name("status"), SQLDataType.VARCHAR(20).nullable(false).defaultValue(DSL.field(DSL.raw("'ACTIVE'::character varying"), SQLDataType.VARCHAR)), this, "");
    ...
    /**
     * The column <code>public.refresh_tokens.used_at</code>.
     */
    ...
    public List<ForeignKey<RefreshTokensRecord, ?>> getReferences() {
        return Arrays.asList(Keys.REFRESH_TOKENS__REFRESH_TOKENS_TOKEN_FAMILY_ID_FKEY);
    }

    private transient TokenFamiliesPath _tokenFamilies;

    /**
     * Get the implicit join path to the <code>public.token_families</code>
     * table.
     */
    public TokenFamiliesPath tokenFamilies() {
        if (_tokenFamilies == null)
            _tokenFamilies = new TokenFamiliesPath(this, Keys.REFRESH_TOKENS__REFRESH_TOKENS_TOKEN_FAMILY_ID_FKEY, null);

        return _tokenFamilies;
    }

    @Override
    public List<Check<RefreshTokensRecord>> getChecks() {
        return Arrays.asList(
            Internal.createCheck(this, DSL.name("chk_refresh_token_status"), "(((status)::text = ANY ((ARRAY['ACTIVE'::character varying, 'USED'::character varying, 'REVOKED'::character varying])::text[])))", true)
        );
    }
